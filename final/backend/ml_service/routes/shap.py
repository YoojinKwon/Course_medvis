...
def patient_shap(subject_id):
    ...
    patch_importance_arr = abs_vals.mean(axis=0)  # (16,)
    patch_importance = patch_importance_arr.tolist()

    # Generate bands: top 5 important patches as highlight regions
    top_indices = np.argsort(-patch_importance_arr)[:5]  # Top 5 patches
    bands = []
    for idx in sorted(top_indices):  # Sort by position, not importance
        start = round(idx / 16, 4)
        end = round((idx + 1) / 16, 4)
        importance = float(patch_importance_arr[idx])
        bands.append({
            "start": start,
            "end": end,
            "importance": round(importance, 6),
            "description": f"Patch {idx + 1}/16"
        })
    ...
    return jsonify({
        ...
        "bands": bands,                       # Top 5 patches as bands
        "vmax": round(vmax, 6),
    })
