...
from config import (
    ...
    RISK_THRESHOLDS, ALLOWED_SUBJECTS_JSON, NOTES_DB,
)
...
class ModelStore:
    ...
    def __init__(self):
        ...
        # Fix sklearn version compatibility: add missing multi_class attribute
        ...
        if NOTES_DB.exists():
            try:
                self.notes_db = sqlite3.connect(str(NOTES_DB), check_same_thread=False)
                self.notes_db.row_factory = sqlite3.Row
                print(f"  Notes DB : {NOTES_DB.name} loaded", flush=True)
            except Exception as e:
                print(f"  Notes DB : Failed to load - {e}", flush=True)
        else:
            print(f"  Notes DB : NOT FOUND at {NOTES_DB}", flush=True)
    ...
    def get_clinical_notes(self, subject_id: str) -> dict:
        """Extract clinical notes from SQLite DB and parse sections"""
        ...
        try:
            # Convert "p14629329" format to numeric 14629329
            numeric_id = int(subject_id.replace('p', ''))

            cur = self.notes_db.cursor()
            cur.execute("SELECT text FROM notes WHERE subject_id = ? LIMIT 1", (numeric_id,))
            row = cur.fetchone()
            ...
            # Extract Allergies
            allergy_match = re.search(r'Allergies:\s*(.*?)(?=\n\n|Chief Complaint|History)', text, re.DOTALL | re.IGNORECASE)
            if allergy_match:
                allergy_text = allergy_match.group(1).strip()
                for line in allergy_text.split('\n'):
                    line = line.strip()
                    if line and len(line) > 1:
                        result['allergies'].append(line)

            # Extract Discharge Diagnosis
            diag_match = re.search(r'Discharge Diagnosis:(.*?)(?=Discharge Condition|Discharge Medications|$)', text, re.DOTALL | re.IGNORECASE)
            if diag_match:
                diag_text = diag_match.group(1).strip()
                if 'PRIMARY' in diag_text:
                    diag_text = re.sub(r'.*?PRIMARY DIAGNOSES.*?={5,}', '', diag_text, flags=re.IGNORECASE | re.DOTALL)
                for line in diag_text.split('\n'):
                    line = line.strip()
                    if line and len(line) > 3 and not line.endswith('='):
                        result['diagnosis'].append(line)

            # Extract Discharge Condition
            cond_match = re.search(r'Discharge Condition:(.*?)(?=Discharge Instructions|Discharge Medications|Followup|$)', text, re.DOTALL | re.IGNORECASE)
            if cond_match:
                cond_text = cond_match.group(1).strip()
                for line in cond_text.split('\n'):
                    line = line.strip()
                    if line and ':' in line and len(line) > 5:
                        result['discharge_status'] += line + ' | '
                result['discharge_status'] = result['discharge_status'].rstrip(' | ')

            # Extract Brief Hospital Course
            course_match = re.search(r'Brief Hospital Course:(.*?)(?=Discharge Diagnosis|Discharge Medications|$)', text, re.DOTALL | re.IGNORECASE)
            if course_match:
                course_text = course_match.group(1).strip()
                sentences = [s.strip() for s in course_text.split('.') if s.strip()][:3]
                result['hospital_course'] = '. '.join(sentences) + '.' if sentences else ''

            # Extract Discharge Medications
            meds_match = re.search(r'Discharge Medications:(.*?)(?=Discharge|Followup|$)', text, re.DOTALL | re.IGNORECASE)
            if meds_match:
                meds_text = meds_match.group(1).strip()
                for line in meds_text.split('\n'):
                    line = line.strip()
                    if re.match(r'^\d+\.\s+', line):
                        med = re.sub(r'^\d+\.\s+', '', line)
                        if med and len(med) > 2:
                            result['medications'].append(med)

            # Extract Followup Instructions
            followup_match = re.search(r'Followup Instructions:(.*?)(?=\n\n|$)', text, re.DOTALL | re.IGNORECASE)
            if followup_match:
                followup_text = followup_match.group(1).strip()
                sentences = [s.strip() for s in followup_text.split('.') if s.strip()][:2]
                result['followup'] = '. '.join(sentences) + '.' if sentences else ''

            return result

        except Exception as e:
            print(f"[ModelStore] Error parsing notes for {subject_id}: {e}", flush=True)
            return {
                'diagnosis': [],
                'discharge_status': '',
                'hospital_course': '',
